...
class TicketRepository {
  final String baseUrl = "http://192.168.2.153:8000/api"; // URL backend Anda
  ...
  Future<void> createOrder(
    ...
    final url =
        'http://192.168.2.152:8000/api/store'; // Ganti dengan URL endpoint yang sesuai
    final token =
  ...
  Future<void> checkout(String orderId) async {
    final response = await http.post(
      Uri.parse('http://192.168.2.152:8000/api/payment/$orderId'),
      headers: {
        'Content-Type': 'application/json',
        'Authorization':
